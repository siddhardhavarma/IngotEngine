...
| `TileSetEditorViewController.swift` | The Tile Sets window (Tiles toolbar button): saved sets list (+/−), atlas image popup, tile size fields — the atlas grid (cols × rows) is AUTO-COMPUTED from the image's pixel size ÷ tile size (editable override) — a "Click assigns" popup (Solid or a named tile CATEGORY, e.g. Ground/Hazards/Decor, each color-coded), and a large clickable atlas preview. Also home of `TileAtlasView`, reused by the Inspector's paint palette. Saves to tilesets.json. |
...
`SpriteNode.characterName` binds a sprite to a character's clip set: attach via the Inspector's Character dropdown, double-clicking the character in the Asset Library (Animations filter), or the AI's `setCharacter`. Attachment scopes script lookups to that character and auto-plays its "idle" clip on scene start (when one exists and no default is set). Serialized with the scene. In design mode (no updates run) the editor calls `showAnimationPreview()` — resting clip's sheet, frozen on frame 0 — on scene load and whenever the character/default animation changes, so the sprite in the viewport looks like what Play will show.
...
In the Tiles window: pick the atlas image and enter the tile size — the atlas grid (cols × rows) is computed automatically from the image's pixel size (override if needed). Choose what clicking assigns via the "Click assigns" popup: **Solid (collision)** or a named **category** ("Ground", "Hazards", "Decor" — like a character groups its animation clips; New Category… creates one), then CLICK cells in the preview. Solid = red, categories get their own colors. Save as a named set ("Terrain"). Apply it to any TileMapNode via the Inspector's Tile Set popup, double-click in the Asset Library, or the AI (`configureTileMap` with `"tileSet": "Terrain"`). Applying copies atlas + grid + tile size + solids onto the node in one step; then paint using the Inspector's clickable palette (same color coding; the selected paint tile gets a yellow border). Categories are editorial — the engine only reads solidTiles. Sets live in tilesets.json; scenes stay self-contained so exports need nothing extra. One atlas per tile map: packs with several sheets (tiles/stones/vegetation) = one tile set per sheet, painted on separate TileMap nodes layered by z-index.
